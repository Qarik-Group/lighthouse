...
function login_to_cf_uaa() {
	...
	echo "Dumping UAA credentials..."
	dump=$($CURL --path=/api/v0/deployed/products)
    echo $dump

	echo "Getting UAA credentials..."
	cf_id=$($CURL --path=/api/v0/deployed/products | jq -r '.[] | select(.type == "cf") | .guid')
	echo "Dumping cf client credentials..."
	...
	dump=$($CURL --path=/api/v0/deployed/products/$cf_id/credentials/.uaa.admin_credentials)
    echo $dump
...
opsman_url="10.213.9.1"
pcf_opsman_admin_username="look in snw-deployments"
pcf_opsman_admin_password="look in snw-deployments"
login_to_cf_uaa
